function[cellMasks, cellIndex, removed, active, timeSeries, it] = ...
        segment(phi_0, video, radius, algorithmPars)
     
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%                          
%%%%%%%%%%%%% Inputs %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% Inputs
% phi_0         : M x N x K initialsation. Each sheet phi_0(:,:,ii) is the  
...
% algorithmPars : Struct. Most important
 
%%% Outputs
% cellMasks     : M x N x K' array. Each sheet cellMasks(:,:,ii) is the  
%                 binary mask of one detected ROI. 
% cellIndex     : K' x 1 array. Entry ii is the position of the iith
%                 resulting mask in the initialisation. 


%%% Ensure video is in correct range [0,1] and format (double)
%video = double(video);
...
if isfield(algorithmPars, 'overlap_radius')
    rad        = algorithmPars.overlap_radius;
    se_overlap = strel('square', rad);
else
...
timeSeries     = zeros(cell_num, t_len);
...
removed.too_small_num   = 0;
removed.too_small_ids   = 0;
removed.too_big_num     = 0;
removed.too_big_ids     = 0;
removed.merged_num      = 0;
removed.merged_ids      = 0;
removed.degenerate_num  = 0;
removed.degenerate_ids  = 0;
removed.overlapping_num = 0;
removed.overlapping_ids = 0;
...
while ii <= size(phi,3)
   ...
   if ~any(currentMask)
        ...
        active(ii)             = [];
        activity(ii, :)        = [];
        removed.degenerate_num = removed.degenerate_num + 1;
        removed.degenerate_ids = [removed.degenerate_ids, cellIndex(ii)];
        cellIndex(ii)          = [];
        if notifications_on
...
while and(it < maxIt, any(active))
    ...
    if and(isfield(algorithmPars, 'mergeDuring'), length(cellIndex) > 1)
        ...
        while and(~isempty(p), ~isempty(q))
            ...
            if any(overlap(:))
                ...
                removed.merged_num = removed.merged_num + 1;
                removed.merged_ids = [removed.merged_ids, cellIndex(merge_remove)];
                removed.preMergeMasks{merge_counter,1} = phi(:,:,p(1))<0;
                removed.preMergeMasks{merge_counter,2} = phi(:,:,q(1))<0;
    ...
    while ii <= size(phi,3)
       ...
       if active(ii)  
           ...
           regularisationVelocity = distanceRegularisation(phiUpdate);
           phiUpdate              = phiUpdate + ...
                                    timestep * region(:,:,ii).*...
           ...
           if and(and(~degenerate, any(onlyThisMask(:))), mask_size < maximumSize)
               ...
               if and(it > noChangeNum, activity(ii,:) < delta)
                    if mask_size < minimumSize
                        removed.too_small_num = removed.too_small_num + 1;
                        removed.too_small_ids = [removed.too_small_ids, cellIndex(ii)];
                        cellIndex(ii)         = [];
                        phi_removed           = 1;
                        phi(:,:,ii)           = [];
                        timeSeries(ii,:)      = [];
                        ...
                        if isfield(algorithmPars, 'no_PSF')
                            timeSeries(ii,:) = extractTimeSeries(onlyThisMask, mods,video_reshaped, t_len);
                             ...
                             timeSeries(ii,:) = extractTimeSeries(phiUpdate<0, mods, video_reshaped, t_len);
                         end
                        if notifications_on
               ...
               if ~any(onlyThisMask(:))
                   removed.overlapping_num = removed.overlapping_num + 1;
                   removed.overlapping_ids = [removed.overlapping_ids, cellIndex(ii)];
                   if notifications_on
                        disp(['Removed ROI ',num2str(ii), ' completely overlapping.']);
                   end
               elseif mask_size > maximumSize
                   removed.too_big_num = removed.degenerate_num + 1;
                   removed.too_big_ids = [removed.degenerate_ids, cellIndex(ii)];
                   if notifications_on
                        disp(['Removed ROI ',num2str(ii), ' degenerate.']);
                   end
           ...
           if isfield(algorithmPars, 'no_PSF')
               ...
               timeSeries(ii,:) = extractTimeSeries(phi(:,:,ii)<0, mods, video_reshaped, t_len);
           end
...
phi(:,:,outsized)         = [];
timeSeries(outsized,:)    = [];
removed.too_small_num     = removed.too_small_num + nnz(outsized);
removed.too_small_ids     = [removed.too_small_ids, find(outsized)'];
...
function [V] = cellSimilarityVelocity(video_dim, phi, n,...
    ...
    if any(narrowbandNoOverlap(:))
        ...
        raw_x        = video_reshaped(vid_loc(:));
        raw_x        = reshape(raw_x, length(loc), t_len);
        V_no_overlap = compare(raw_x, n, a);
        V(narrowbandNoOverlap) = V_no_overlap;  
    end
    
    % Calculate the velocity where there are overlapping cells
    if nnz(narrowbandWithOverlap) > 0
        ...
        for j = 1:length(loc) %the bit in here probably isnt optimal
            J              = find(phi(loc_x(j),loc_y(j),:) < 0);
            J(J == ii)     = [];
            b              = sum(timeSeries(J,:),1);  % time series of cells that x is currently in
            x              = squeeze(raw_x(j,:));     % time series of current pixel
...
sd        = sd*c0;
sd(sd>c0) = c0;
if isfield(algorithmPars, 'sdf_prev')
    sd(sd<c0) = -c0;
else
    sd(sd<-c0) = -c0;
end
